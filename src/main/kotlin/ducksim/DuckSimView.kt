...
class DuckSimView(private val duckPond: DuckPond) : JPanel() {
    ...
    private fun paintQuack(g: Graphics) {
        ...
        g.color = Color.BLACK
        val quack: String = duckPond.currentDuck!!.quackBehavior.getQuack()
    ...
    private fun paintDuck(g: Graphics, duck: Duck, rect: Rectangle) {
        ...
        if (duck.isOnDSWC) {
            // If the duck join DSWC, put a magenta w in the lower left portion and welcome text above its name
            ...
            welcomeText = if (duck.isFree) {
                "Welcome!"
            } else {
                "Beware!"
            }
            drawCenteredTextInRectangle(g, text = welcomeText, rectangle = rect, yOffset = 35)
            duck.swim()
        }
